...
**Status: PASS (one incremental boundary completed)**

Phase 16 was intentionally limited to one low-risk extraction. No broad rewrite,
API change, visual redesign, or backend split was performed.

## Scope and baseline

Before this phase, the frontend entry point was a single file:

```text
web/src/main.tsx       2,548 lines (baseline)
web/src/styles.css     5,465 lines (unchanged)
```

`main.tsx` contained routing, page components, modal components, layout/splitter
behavior, API calls, and pure display/formatting helpers. The backend structure
was not changed because this phase's verified maintenance risk is concentrated
in the frontend monolith and no backend boundary was required for this first
step.
...
1. Added `web/src/shared/utils.ts` with the 15 existing pure helper functions.
2. Replaced the duplicate local helper definitions in `web/src/main.tsx` with a
   single named import block.
3. Preserved the helper implementations byte-for-byte in behavior, including:
...
4. Did not modify `web/src/styles.css`, Python source, API contracts, database
   access, authentication, or workflow transitions.

The resulting `main.tsx` is 2,458 lines, a reduction of 90 physical lines,
while the runtime responsibilities remain in their original component owners.
...
1. shared status chips, toast, and data-state primitives;
...
This phase stops after the first shared-utility extraction and its verification.
